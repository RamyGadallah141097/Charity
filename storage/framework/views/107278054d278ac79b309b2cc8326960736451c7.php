<div class="modal-body">
    <form id="addForm" class="addForm" method="POST" enctype="multipart/form-data"
        action="<?php echo e(route('Donations.store')); ?>">
        <?php echo csrf_field(); ?>
        ...
        <div class="form-group" id="select_donor_container">
            ...
            <select name="donor_id" id="donor_name" class="form-control">
                ...
                <?php $__currentLoopData = $donors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $donor): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($donor->id); ?>"><?php echo e($donor->name); ?></option>
...
<script>
    $(document).ready(function() {
        ...
        $('#search_donor').on('keyup', function() {
            let query = $(this).val();
            $.ajax({
                url: "<?php echo e(route('search.donor')); ?>",
                method: 'GET',
                data: {
                    donor_names: query
                },
                    ...
                    if (response.length > 0) {
                        ...
                        $("#create_donor").append(`
                            <button class="btn btn-secondary btn-icon text-white addBtn" >
                               <a class="text-white" href="<?php echo e(route('donors.index')); ?>"> <i class="fe fe-plus"></i> اضافة متبرع جديد</>
                            </button>
                        `);
...
<script>
    $(document).ready(function() {
        $('select[id="donor_name"]').on('change', function() {
                ...
                $.ajax({
                    url: '<?php echo e(route('get_donor_phone', ':donor_name')); ?>'.replace(":donor_name",
                        donor_name),
                    type: "GET",
                    data: {
...
<?php /**PATH C:\xampp\htdocs\new-zakat\resources\views/admin/donations/parts/create.blade.php ENDPATH**/ ?>